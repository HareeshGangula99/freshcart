...
const Profile: React.FC = () => {
  ...
  const closeChat = () => { setChatOrder(null); setMessages([]); setInput(''); };
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div className="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
        ...
        <div className="card-body" style={{ marginTop: '-40px' }}>
          <div className="d-flex align-items-end gap-3 mb-3">
            <div className="rounded-4 d-flex align-items-center justify-content-center text-white fw-bold fs-4 border border-4 border-white shadow-lg" style={{ width: '80px', height: '80px', background: 'linear-gradient(135deg, #059669, #10b981)' }}>
              {user?.name?.[0]?.toUpperCase()}
            </div>
            <div className="pb-1">
              <h4 className="fw-bold mb-0" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{user?.name}</h4>
          ...
          <div className="d-flex flex-column gap-3">
            {orders.map(order => (
              <div key={order._id} className="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div className="card-body p-3 d-flex align-items-center justify-content-between cursor-pointer" onClick={() => setExpandedOrder(expandedOrder === order._id ? null : order._id)}>
                  ...
                  <div className="d-flex align-items-center gap-2">
                    <span className="fw-bold text-success" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>₹{order.totalAmount}</span>
                  ...
                  <div className="card-body border-top p-3 pt-3">
                    ...
                    <div className="d-flex flex-column gap-2 mb-3">
                      {order.products?.map((item: any) => (
                        <div key={item._id} className="d-flex align-items-center gap-3 p-2 rounded-3" style={{ background: '#f9fafb' }}>
                          {item.productId?.imageURL && <img src={item.productId.imageURL.startsWith('http') ? item.productId.imageURL : `${API_BASE}${item.productId.imageURL}`} alt="" className="rounded-2" style={{ width: '40px', height: '40px', objectFit: 'cover' }} />}
                          <div className="flex-grow-1">
                            <p className="mb-0 fw-medium" style={{ fontSize: '13px' }}>{item.productId?.name || 'Product'}</p>
                            <small className="text-muted">₹{item.priceAtPurchase} x {item.quantity}</small>
                          </div>
                          <p className="fw-semibold mb-0" style={{ fontSize: '13px' }}>₹{item.priceAtPurchase * item.quantity}</p>
                        </div>
                      ))}
                    </div>
                    ...
                    {(order.orderStatus === 'DISPATCHED' || order.orderStatus === 'OUT_FOR_DELIVERY') && (
                      <div className="d-flex gap-2 mt-3">
                        {!isDeliveryPartner && (
                          <button onClick={() => openTracking(order)} className="btn flex-grow-1 fw-semibold text-white rounded-3 py-2 d-flex align-items-center justify-content-center gap-2" style={{ background: '#7c3aed', fontSize: '13px' }}>
                            <i className="bi bi-geo-alt"></i> Track Order
                          </button>
                        )}
                        <button onClick={() => openChat(order)} className={`btn ${isDeliveryPartner ? 'w-100' : 'flex-grow-1'} fw-semibold text-white rounded-3 py-2 fc-primary d-flex align-items-center justify-content-center gap-2`} style={{ fontSize: '13px' }}>
                          <i className="bi bi-chat-dots"></i>
                          {isDeliveryPartner ? 'Chat with Customer' : 'Chat with Delivery Partner'}
                        </button>
                      </div>
                    )}
                  </div>
      ...
      {/* Chat Modal */}
      {chatOrder && (
        <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-end align-items-md-center justify-content-center p-3" style={{ background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', zIndex: 1060 }}>
          <div className="card border-0 shadow-lg rounded-4 d-flex flex-column animate-scale-in" style={{ width: '100%', maxWidth: '420px', height: '80vh', maxHeight: '480px' }}>
            <div className="card-header d-flex align-items-center justify-content-between border-0 p-3" style={{ background: '#ecfdf5' }}>
              <div className="d-flex align-items-center gap-3">
                ...
                <div>
                  <p className="fw-semibold mb-0" style={{ fontSize: '13px' }}>{isDeliveryPartner ? chatOrder.userId?.name || 'Customer' : 'Delivery Partner'}</p>
      ...
      {/* Tracking Modal */}
      {trackingOrder && (
        <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-end align-items-md-center justify-content-center p-3" style={{ background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', zIndex: 1060 }} onClick={(e) => { if (e.target === e.currentTarget) closeTracking(); }}>
          <div className="card border-0 shadow-lg rounded-4 d-flex flex-column animate-scale-in" style={{ width: '100%', maxWidth: '500px', height: '85vh', maxHeight: '700px' }}>
            {/* Header */}
